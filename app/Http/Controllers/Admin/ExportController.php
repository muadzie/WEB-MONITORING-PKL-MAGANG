...
private function generateKelompokHTML($kelompoks, $request)
{
    ...
    $html .= '<table><thead><tr>';
    $html .= '<th>No</th><th>Nama Kelompok</th><th>Guru Pembimbing</th><th>Perusahaan</th>';
...
private function generateNilaiHTML($kelompok)
{
    ...
    $html .= '<table><thead><tr>';
    $html .= '<th>No</th><th>NISN</th><th>Nama Siswa</th><th>Nilai Guru</th><th>Nilai PT</th><th>Nilai Akhir</th><th>Grade</th>';
    $html .= '<tr></thead><tbody>';
    
    $no = 1;
    foreach ($kelompok->anggota as $anggota) {
        $nilaiDosen = Penilaian::where('kelompok_siswa_id', $anggota->id)->where('penilai', 'dosen')->first();
        $nilaiPt = Penilaian::where('kelompok_siswa_id', $anggota->id)->where('penilai', 'pt')->first();
        $nilaiAkhir = null;
...
private function generateSiswaHTML($siswas, $request)
{
    ...
    $html .= '<table><thead><tr>';
    $html .= '<th>No</th><th>NISN</th><th>Nama</th><th>Email</th><th>Telepon</th><th>Kelompok</th><th>Status Kelompok</th>';
    $html .= '</tr></thead><tbody>';
...
private function generateRekapSiswaHTML($siswas, $request)
{
    ...
    $html .= '<table><thead><tr>';
    $html .= '<th>No</th><th>NISN</th><th>Nama</th><th>Email</th><th>Telepon</th>';
    $html .= '<th>Kelompok</th><th>Kelas</th><th>Jurusan</th><th>Status</th>';
    $html .= '</tr></thead><tbody>';
...
private function generateRekapKelompokHTML($kelompoks, $request)
{
    ...
    $html .= '<style>
        ...
        .header h2 { color: #4472C4; margin: 0; }
        table { border-collapse: collapse; width: 100%; }
        th, td { border: 1px solid #000000 !important; padding: 8px; }
        th { background-color: #4472C4; color: white; }
        tr:nth-child(even) { background-color: #f2f2f2; }
        .footer { margin-top: 20px; }
        .status-aktif { color: green; font-weight: bold; }
...
private function generateRekapLogbookHTML($logbooks, $request)
{
    ...
    $html .= '<table><thead><tr>';
    $html .= '<th>No</th><th>Tanggal</th><th>Siswa</th><th>NISN</th><th>Kelompok</th>';
...
private function generateRekapTahunanHTML($kelompokPerBulan, $logbookPerBulan, $laporanPerBulan, $tahun)
{
    ...
    for ($i = 0; $i < 12; $i++) {
        ...
        $html .= '<tr>';
        $html .= '<td><strong>' . $bulan[$i] . '</strong></td>';
        $html .= '<td>' . $dataKelompok[$i] . '</td>';
        $html .= '<td>' . $dataLogbook[$i] . '</td>';
        $html .= '<td>' . $dataLaporan[$i] . '</td>';
        $html .= '</tr>';
    }
    $html .= '<tr class="total-row">';
    $html .= '<td><strong>TOTAL</strong></td>';
    $html .= '<td><strong>' . $totalKelompok . '</strong></td>';
    $html .= '<td><strong>' . $totalLogbook . '</strong></td>';
    $html .= '<td><strong>' . $totalLaporan . '</strong></td>';
    $html .= '</tr>';
    $html .= '</tbody></table>';
